...
export function BodyShopServices() {
    ...
    useEffect(() => {
        const ctx = gsap.context(() => {
            ...
            if (bsText && bsImage) {
                gsap.fromTo(bsText,
                    { x: -50, opacity: 0 },
                    { x: 0, opacity: 1, duration: 1, scrollTrigger: { trigger: bodyShopSectionRef.current, start: "top 70%" } }
                );
                gsap.fromTo(bsImage,
                    { x: 50, opacity: 0 },
                    { x: 0, opacity: 1, duration: 1, scrollTrigger: { trigger: bodyShopSectionRef.current, start: "top 70%" } }
    ...
    return (
        <div ref={containerRef} className="flex flex-col w-full">
            ...
            <div ref={bodyShopSectionRef} className="flex flex-col lg:flex-row min-h-[600px] overflow-hidden bg-white mb-20">
                {/* Text Side (Left) */}
                ...
                <div className="container mx-auto px-6">
                    <div className="text-center mb-16 space-y-4">
                        <h2 className="font-heading text-3xl md:text-5xl font-bold tracking-tight text-secondary uppercase">
                            Our Specialized Bodyshop Services
                        </h2>
                        <p className="font-sans text-muted-foreground max-w-2xl mx-auto">
                            Expert restoration and repair aimed at perfection.
                        </p>
                    </div>
                    ...
                    <div ref={gridRef} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {BODYSHOP_SERVICES.map((service, index) => {
                            ...
                            return (
                                <div key={service.id} className="flip-card-bs group h-[400px] w-full [perspective:1000px]">
