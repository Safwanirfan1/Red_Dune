...
export function WorkshopServices() {
    ...
    useEffect(() => {
        const ctx = gsap.context(() => {
            ...
            const workshopText = workshopSectionRef.current?.querySelector(".workshop-text");
            const workshopImage = workshopSectionRef.current?.querySelector(".workshop-image");

            if (workshopText && workshopImage) {
                gsap.fromTo(workshopText,
                    { x: -50, opacity: 0 },
                    { x: 0, opacity: 1, duration: 1, scrollTrigger: { trigger: workshopSectionRef.current, start: "top 70%" } }
                );
                gsap.fromTo(workshopImage,
                    { x: 50, opacity: 0 },
                    { x: 0, opacity: 1, duration: 1, scrollTrigger: { trigger: workshopSectionRef.current, start: "top 70%" } }
                );
            }
    ...
    return (
        <div ref={containerRef} className="flex flex-col w-full">
            ...
            <div ref={workshopSectionRef} className="flex flex-col lg:flex-row min-h-[600px] overflow-hidden bg-white mb-20">
                {/* Text Side (Left) */}
