...
export function EngineeringServices() {
    ...
    useEffect(() => {
        const ctx = gsap.context(() => {
            ...
            const engImage = engineeringSectionRef.current?.querySelector(".eng-image");

            if (engText && engImage) {
                gsap.fromTo(engText,
                    { x: -50, opacity: 0 },
                    { x: 0, opacity: 1, duration: 1, scrollTrigger: { trigger: engineeringSectionRef.current, start: "top 70%" } }
                );
                gsap.fromTo(engImage,
                    { x: 50, opacity: 0 },
                    { x: 0, opacity: 1, duration: 1, scrollTrigger: { trigger: engineeringSectionRef.current, start: "top 70%" } }
                );
            }
    ...
    return (
        <div ref={containerRef} className="flex flex-col w-full">
            ...
            <div ref={engineeringSectionRef} className="flex flex-col lg:flex-row min-h-[600px] overflow-hidden bg-white mb-20">
                {/* Text Side (Left) */}
                ...
                <div className="container mx-auto px-6">
                    ...
                    <div ref={gridRef} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {ENGINEERING_SERVICES.map((service, index) => {
                            ...
                            return (
                                <div key={service.id} className="flip-card-eng group h-[400px] w-full [perspective:1000px]">
                                    <div className="relative h-full w-full transition-all duration-700 [transform-style:preserve-3d] group-hover:[transform:rotateY(180deg)]">
                                        ...
                                        <div className="absolute inset-0 h-full w-full bg-secondary rounded-xl shadow-xl overflow-hidden [transform:rotateY(180deg)] [backface-visibility:hidden]">
                                            ...
                                            <div className="relative z-20 h-full flex flex-col items-center justify-center text-white p-8 text-center space-y-6">
                                                ...
                                                <h3 className="font-heading text-2xl font-bold uppercase">{service.title}</h3>
                                                <Link href="#contact">
